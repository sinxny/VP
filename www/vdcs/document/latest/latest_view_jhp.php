            ...
            .then(function(response) {
                if((response["data"]["Message"]) == "Success") {
                    ...
                    $("#btnConfirm").on("click", function() {
                       ...
                       var f = document.getElementById("frmDownload");
                       f.action = url;
                       //f.submit();
                    });
                }
...
function goTestForm()
{
    ...
    var url = "/api/vdcs/?api_key=d6c814548eeb6e41722806a0b057da30&api_pass=BQRUQAMXBVY=&model=LATEST_ZIP_DOWNLOAD&jno=17624";
    var options = { 
            url: url 
    ...
    $("#testForm").submit( function() { //submit이 발생하면
...
function goTestForm04(){
    $("#modalLoading").modal("show");
    
    var callUrl = "/api/vdcs/?api_key=d6c814548eeb6e41722806a0b057da30&api_pass=BQRUQAMXBVY=&model=LATEST_ZIP_DOWNLOAD&jno=17045";//17624-BH1, 17045-LP(II), 16601-MDI
    var callUrl2 = callUrl + (/\?/.test(callUrl) ? "&" : "?") + new Date().getTime();
    location.href = callUrl2;
    ifHasChanged(callUrl2, function (modified, visit) {
      console.log(
...
function goTestForm03(){
    ...
    req.onload = getHeaderTimeTest;
    req.send();
     
    //$("#modalLoading").modal("hide");
}
function goTestForm02(){
    alert("Go2");
    $("#modalLoading").modal("show");
    var callUrl = "/api/vdcs/?api_key=d6c814548eeb6e41722806a0b057da30&api_pass=BQRUQAMXBVY=&model=LATEST_ZIP_DOWNLOAD&jno=17045";//17624-BH1, 17045-LP(II), 16601-MDI
    var callUrl2 = callUrl + (/\?/.test(callUrl) ? "&" : "?") + new Date().getTime();
    ...
    // 이벤트 리스너를 먼저 등록해준다
    req.onload = () => {
        //console.log(`The transfer is completed: ${req.status} ${req.response}`);
        $("#modalLoading").modal("hide");
    };
    req.onabort = () => {
    ...
    req.open('HEAD', callUrl2);
    //req.overrideMimeType("application/octet-stream; charset=x-user-defined");
    //req.responseType = "arraybuffer";
    req.send();
}
</script>
<iframe name="ifrm" id="ifrm" width="0" height="0" scrolling="no" style="margin:0" src=""></iframe>
<form name="testForm" id="testForm" method="post" action="" target="ifrm">
    <input type="button" value="GO" onclick="goTestForm02()" />
</form>
